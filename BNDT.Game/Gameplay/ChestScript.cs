...
{
    public class ChestScript : SyncScript
    {
        ...
        protected void UpdateAnimation()
        {
            ...
            animationTime += (float)Game.UpdateTime.Elapsed.TotalSeconds;

            //Top.Transform.Rotation.X = (float)MathUtil.Lerp(0, -0.25, 4 * (float) Utils.Smoothstep(0, 0.7, 2 * (double) Math.Sin(animationTime)));

            Top.Transform.RotationEulerXYZ = new Vector3((float)MathUtil.Lerp(0, -0.25, 4 * (float)Utils.Smoothstep(0, 0.7, 2 * (double)Math.Sin(animationTime))), 0, 0);
